...
Common EAS ground-based arrays such as the Telescope Array~\cite{abu12}, Yakutsk EAS array~\cite{}\todo{\Russian Ссылка на Якутск} or the Tunka-133~\cite{}\todo{\Russian Ссылка на Тунку} detector are the ground-based structures that are spread over up to hundreds~\cite{abu12} square kilometers. Significant effort is needed in order to install sensitive elements of such vast arrays and to keep them network connected, power-supplied and time-synchronized. More effort is needed for the regular calibration of detector stations and atmosphere parameter control over vast area. 

On the other hand common imaging air Cherenkov telescopes (IACT, like HESS~\cite{}\todoi{\Russian Ссылка на HESS} or MAGIC~\cite{}\todo{\Russian Ссылка на MAGIC}) are relatively compact systems that have good calibration means, high integrity power supply, atmosphere transparency control and so on.
...
%\todoi{Нужно уточнить точность GPS}
...
\hspace{6.5 mm}* 2 m accuracy according to the manufacturer provided data, 1 m accuracy according to our own analysis (see. below in section )

\hspace{5 mm}** 4 m accuracy according to the manufacturer provided data, 2 m accuracy according to our own analysis (see. below in section )
\normalsize
\end{table*}

\todo{\Russian В таблицу необходимо добавить датчик давления в аэростате CPCL04DFC с диапазоном 0-1кПа и точностью ~3Па.}
...
%{\Russian 
%
%Помимо основной системы регистрации с детектором черенковского света на установке СФЕРА2 установлено большое количество вспомогательных датчиков для обеспечения её работы. Вспомогательные датчики устанавливаются внутри блока с электроникой, снаружи и на мозаике ФЭУ. Перечень датчиков с диапазоном, точностью и периодом измерения приведен в таблице~\ref{tab:telemetry_sensors}. Внутри блока электроники измеряется температура плат FADC для управления вентиляторами охлаждения, напряжения вторичных источников питания и питающего аккумулятора, а так же ток потребления с аккумулятора, дифференциальный датчик давления в аэростате. К внешним датчикам относятся GPS-приёмник, датчики температуры и атмосферного давления. На мозаике ФЭУ установлен инклинометр и магнитометр для определения положения установки в вертикальных плоскостях и горизонтальной. Кроме того, на каждом источнике питания измеряются напряжения питания ФЭУ, анодный ток и температура оптического модуля.
%}
...
{\Russian
Точность определения координат датчиком GPS была определена по данным GPS в то время, когда установка находилась неподвижно на льду озера. Были построены посуточные распределения значений координат и высоты за 20 дней в 2012 и 2013 годах. Средняя сигма (а на самом деле медианная) этих распределений принята за точность определения координат и высоты датчиком GPS.
}
...
\todo{\Russian  Описать в два слова, зачем оно тут в таком количестве регистрировалось}
...
\centering
    ...
    \caption{The SPHERE-2 retina 2012 for figure~\ref{fig:2012-3_currents}.}
    %\todo{Выберите одну из двух.}
...
%\todoi{Нужна картинка, где закрашены эти ФЭУ, нужным цветом каждый} 
...
\todo{\Russian Нужна новая картинка оптической части детектора. Надо со снегом, изображающим поверхность, потому что рассеяние на нём входит в схему расчёта.}

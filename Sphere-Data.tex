...
\begin{table*}[bth]
\centering
\caption{Telemetry sensors parameters}\todoi{Нужно уточнить точность GPS}
...
% кратко про мозаику
\todo{Сделать ссылку на таблицу~\ref{tab:telemetry_sensors}} 
{\Russian 
...
\todoi{Написать раздел про то, что есть из датчиков в блоке аппаратуры и на мозаике}
...
\todo{Описать в два словыа, зачем оно тут в таком количестве регистрировалось}
...
The PMTs currents variations during the observations generally followed the variation of the background illumination of the snowed lake surface. However in one flight (namely, third flight of the 2012 season) in part of the PMTs\todoi{Нужна картинка, где закрашены эти ФЭУ, нужным цветом каждый}~\ref{fig:2012-3_retina} the abnormal drop of currents was observed.
...
\todo{Нужна новая картинка оптической части детектора. Надо со снегом, изображающим поверхность, потому что рассеяние на нём входит в схему расчёта.}
...
We are grateful to the Lebedev Physical Institute of Russian Academy of
Sciences group (leader S.B.~Shaulov) for assistance in assembling and
testing the electronic equipment and in preparation of expeditions. We
also thank the Baikal-GVD collaboration and G.V.~Domogatsky (Institute
for Nuclear Research, Russian Academy of Sciences) for the support of
the SPHERE experiment at the Baikal Lake scientific station.

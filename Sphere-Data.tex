...
%\hline
\hline
\multirow{6}{*}{1 sec} & \multirow{3}{*}{Detector position} &GPS altitude & 2 &  -1500&--&18000  & m a.s.l.\\
                                                      \cline{3-8}
                       &                              & GPS coordinates & 1 & &---&& m\\
                                                      \cline{3-8}
                       &                              & GPS time (PPS)& 1 & &---&& $\mu$s \\
...
\todo{\Russian Написать раздел про то, что есть из датчиков в блоке аппаратуры и на мозаике}
...
%Точность определения координат датчиком GPS была определена по данным GPS в то время, когда 
%%% ошибка начинается здесь
%установка находилась неподвижно на льду озера. Были построены посуточные распределения значений координат и высоты за 20 дней в 2012 и 2013 годах. Средняя сигма (а на самом деле медианная) этих распределений принята за точность определения координат и высоты датчиком GPS.
}

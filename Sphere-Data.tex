...
We produced such artificial events initiated by 1 PeV protons for a number of models,  passing close to the experimental $T$ and $\rho$ trajectories, and CORSIKA model 11. The results are expressed as ratios of full numbers of Cherenkov photons and lateral distribution functions.
...
Sample volume for each model was 30 showers. Table~\ref{tab:atmmod} shows mean values and variations of Cherenkov photon number ratios for two pairs of CORSIKA atmosphere models (3/4 and 3/11). The corresponding ratios of sample mean lateral distributions for these pairs are shown in Fig.~\ref{fig:3d4} and Fig.~\ref{fig:3d11}, respectively. Pair 3/4 reflects maximum effect of atmosphere model choice from the viewpoint of experimental $T$ and $\rho$ measurements. Pair 3/11 compares the model 11 used in simulations to the model 3 which approximates the experimental data and is the most distant from model 11 in terms of $T$.
Table data clearly states that substantial changes of atmosphere model affect the total number of Cherenkov photons and thus the primary energy estimates not more than by 5\% on average. Conclusions on primary mass estimates are not so clear but the lateral distribution ratio plots indicate some changes in CL LDF of about 5-7\% (7-15\%) near the shower core, closer than 80 m, and about 2-5\% (-2 to +7\%) in 80-150m circle for 3/4 (3/11) pair, which definitely affect our mass-sensitive criterion. More accurate evaluation of the effect will be given elsewhere.

%% влияние атмосферы на высоты генерации света на (Галкин)
%% картинки
\begin{table}[t]
\centering
\caption{Cherenkov number ratios for different CORSIKA atmosphere model pairs: means, variations, relative variations}
\label{tab:atmmod}
\vspace{1pc}
...
\begin{figure}[tb]
\centering
\includegraphics[width=19pc]{figs/3d4.eps}%
%\vspace{-1.0pc}
\caption{Sample mean Cherenkov light lateral distribution ratio for CORSIKA atmosphere model
pair 3/4. Bin size $50m \times 50m$. Sample volume 30 events.}
\label{fig:3d4}
\end{figure}

\begin{figure}[tb]
\centering
\includegraphics[width=19pc]{figs/3d11_20.eps}%
\vspace{-1.0pc}
\caption{Sample mean Cherenkov light lateral distribution ratio for CORSIKA atmosphere model
pair 3/11. Bin size $50\times 50$~m$^2$. Sample volume 30 events.}
\label{fig:3d11}
\end{figure}

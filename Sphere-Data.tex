...
The detector axis at the rest state is oriented vertically and detector observes the snow covered surface just under the detector. During the measurements the detector hang under the balloon and swung and rotated freely. The detector inclination is an important factor for the shower parameters reconstruction as it determine the overall geometry of the experiment. To control the position and rotation of the detector an inclinometer and a digital compass were installed on the PMT mosaic control board. 
...
The accuracy position determination of the GPS sensor in rest state was calculated from GPS data at a time when the detector was stationary on the lake ice. Daily distributions of the detector coordinates and altitude were plotted for 20 days in 2012 and 2013. The average sigma (actually the median) of these distributions is taken as the accuracy of the GPS sensor's determination: 2~m for coordinates and 1~m for altitude. \todoi{Подумать про график.}
...
%\todoi{Done. Исправить количество триггеров 2008-2009}
}
...
\begin{figure}[tb]
    ...
    \caption{Detector inclination vs the detector drift from the start point to the detector altitude ratio which roughly translates into the tether inclination angle.\todoi{Дописать про разбров с полёте 2013-3 и испорченную погоду.}}
    \todoi{Нет ссылки на эту картинку в тексте.}
...
\begin{figure}[tb]
    ...
    \caption{The SPHERE-2 detector drift in 2012-3 flight. Mosaic projection to the show surface is given for the time 15:47 UTC. Detector GPS position at that moment is indicated by the cross.}\todoi{Убрать bold, уменьшить шрифт. Фонт DejaVu Sans.}
...
\begin{minipage}[t]{0.48\textwidth}
    ...
    %\todoi{Done! Добавить данные на рисунки}
\label{fig:density}
\end{minipage}
\end{figure}

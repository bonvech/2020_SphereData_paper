...
\centering
    ...
    \caption{The detector optical scheme.}\todoi{Убрать буквы в середине рисунка}
\label{fig:optics}
\end{figure}
...
\begin{figure*}[tb]
    ...
    \caption{The detector inclination according to the inclinometer sensor during several flights in 2011-2013.}\todoi{Добавить 2013-3}
...
\todoi{Описать Fig.~\ref{fig:drift-inclination}} Detector inclination vs the detector drift from the start point to the detector altitude ratio which roughly translates into thetether inclination angle.
...
\begin{figure}[tb]
    ...
    \caption{The central PMT currents in different flights.}\todoi{Может быть, поднормировать токи на одно напряжение? Чтоб не объяснять почему тут перепады?}
\label{fig:current}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\todoi{Привести цвета на нижнем рисунке в соответствие с цветами на верхнем и среднем рисунках. Ответ: они специально другие, т.к. обозначают принципиально другие величины.}
...
Calculated experimental points for $T$ and $\rho$ in each flight are shown in Fig.~\ref{fig:massoverburden} and Fig.~\ref{fig:density}. CORSIKA atmospheres are shown with dashed lines except for the profile that was adopted in the preliminary modelling, which is shown with solid black line. \todoi{solid black line на картинке отсутствует!!}
